...
export default function Dashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    if (!user) return; // Early exit inside the callback
    try {
      const [productRes, userRes] = await Promise.all([getProducts(""), getAllUsers()]);
      const products = productRes.data || [];
      const users = userRes.data || [];

      // --- Compute summary, charts, etc. ---
      ...
      setSummary({ totalRevenue, totalSales, totalEmployees, totalTeams, topTeam });
      ...
      setMonthlyRevenue(months.map(m => ({ month: m, revenue: monthlyMap[m] || 0 })));
      ...
      setProgressData(Object.keys(progressMap).map(key => ({ name: key, value: progressMap[key] })));
      ...
      setRoleData(Object.keys(roleMap).map(role => ({ role, count: roleMap[role] })));
    } catch (err) {
      console.error("Dashboard load error:", err);
    }
  }, [user]);
  ...
  if (!user) {
    return <Box sx={{ p: 3 }}><Typography variant="h6">Not logged in</Typography></Box>;
  }
